...
call_tasks = {
    '449697e5-ef8d-436f-a1f8-7374ba26c305': {'status': 'pending', 'customer_id': 'CUST999', 'customer_name': 'Kushal Sharma', 'customer_phone_number': '+917417119014', 'loan_last4': 'N999', 'emi_amount': '₹15,000', 'due_date': '15th July', 'loan_id_full': 'LN999XYZ', 'current_language': '2'},
    '935cf83e-2bc4-4b85-b4ba-657105fe67d1': {'status': 'pending', 'customer_id': 'CUST001', 'customer_name': 'Arun', 'customer_phone_number': '+917', 'loan_last4': 'M123', 'emi_amount': '₹5,000', 'due_date': '10th July', 'loan_id_full': 'LN123ABC', 'current_language': '1'},
...
def require_task(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not task_id or task_id not in call_tasks:
            ...
            # This message is returned if the task_id is missing or invalid.
            # This often happens if the server restarts and the in-memory 'call_tasks' is cleared.
            response.say("I'm sorry, I cannot find your call information. This may be due to a system update. Goodbye.", voice="Polly.Raveena", language="en-IN")
            response.hangup()
            return str(response)
...
def send_whatsapp_summary(task_id, to_number, customer_name, loan_id, emi_amount, outcome):
    """
    Sends a summary of the call outcome to the agent via WhatsApp.
    """
    if not AGENT_PHONE_NUMBER:
        print("⚠️ Agent phone number not configured. Skipping WhatsApp summary.")
        return
    
    summary_message = (
        f"📞 Call Escalation Summary 📞\n\n"
        f"Customer: {customer_name}\n"
        f"Loan ID: {loan_id}\n"
        f"EMI Due: {emi_amount}\n"
        f"Outcome: {outcome}\n"
        f"Task ID: {task_id}"
    )
    try:
        ...
        print(f"✅ WhatsApp summary sent to agent. SID: {message.sid}")
    except Exception as e:
        print(f"❌ Failed to send WhatsApp summary to agent: {e}")
